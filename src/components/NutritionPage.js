...
export default class NutritionPage {
  ...
  async loadDailyLogs() {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/nutrition/logs/${today}`, {
        headers: {
  ...
  async handleFoodAdd(food) {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/nutrition/log`, {  // Note: changed from /logs to /log
        method: "POST",
        headers: {
  ...
  async handleDeleteLog(logId) {
    if (confirm("Are you sure you want to delete this food log?")) {
      try {
        const token = localStorage.getItem("token");
        const response = await fetch(`${API_BASE_URL}/api/nutrition/logs/${logId}`, {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json'
